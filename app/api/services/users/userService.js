...
module.exports = {
    register: function (req, res, next) {
        ...
        if (errorMessage) {
            ...
            var newUser = new connMas.UserMaster({
            ...
            newUser.save(function (err) {
                ...
                res.json({success: true, statusCode: res.statusCode, message: 'Created successfully', data: {}});
            });
        }
    },
        ...
        connMas.UserMaster.find(query).select(['-password', '-createdBy', '-updatedAt', '-__v'])
                ...
                .exec(function (err, users) {
                    if (err) {
                        ...
                        res.json({success: true, statusCode: res.statusCode, message: "No record found", data: {users: []}});
                    } else {
                        res.json({success: true, statusCode: res.statusCode, message: "Record found!!!", data: {users: users}});
                    }
                });
    },
    getById: function (req, res, next) {
        connMas.UserMaster.findById(req.params.userId).select("-password").select(["-createdAt", "-updatedAt", "-__v"])
                .populate('company', ['-updatedAt', '-dbName', '-dbUsername', '-dbPassword', '-dbPort', '-dbHost', '-createdBy', '-__v'], connMas.CompanyMaster)
                ...
                .exec(function (err, user) {
                    ...
                    if (!user) {
                        ...
                        res.json({
                            ...
                            message: "User Found",
                            data: {
                                user: {_id: user._id, email: user.email, firstname: user.firstname, lastname: user.lastname, active: user.active, createdAt: user.createdAt},
                                company: company,
                                branch: branch,
                                role: {_id: role._id, name: role.name, displayName: role.displayName, active: role.active, default: role.default, createdAt: user.role ? user.role.createdAt : ''},
                                resources: resources,
            ...
            if (err)
                ...
                if (req.body.password && req.body.password !== null) {
                    ...
                    user.save(function (err, user) {
                        if (err) {
                            res.json({success: false, statusCode: res.statusCode, message: "Some problem occured. Try again!!!", data: null});
                        } else {
